#model4; no periodicity; same as model2 with different parameters
# also set priors for the intercept and slope
#OU_amp = 0.5; OU_scale = 1.5 # hyparparameters for OU
#intercept = 0; slope = 0 # basline intercept and slope 
#intercept_treat  = 0.3; slope_treat = 0.07 # treatment effects
#sigma = 0.5
...
stan_file <- normalizePath("Stan Model/Single subject semiparametric ITS4.stan")
fit1 = stan(file = stan_file,data = stan_data )

save.image(file = 'output/single subject Semiparametric ITS model4.Rdata')
write.table(data.frame(summary(fit1)$summary),
            file = 'output/Single subject semiparametric ITS model4-summary',
            sep = '\t', quote = FALSE, col.names = NA)
ggmcmc(ggs(fit1, inc_warmup = TRUE, stan_include_auxiliar = TRUE),
       file = 'output/Single subject semiparametric ITS model4-traceplot.pdf', 
       plot = 'traceplot')
ggmcmc(ggs(fit1), file = 'output/Single subject semiparametric ITS model4-ggmcmc.pdf')
