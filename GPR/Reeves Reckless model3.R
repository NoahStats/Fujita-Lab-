...
fit1 = stan(file = 'Stan Model/Reeves reckless model3.stan', data = stan_data1)
save.image(file = 'output/Reeves reckless model3.Rdata')

library(ggmcmc)
load('output/Reeves reckless model3.Rdata')

write.table(data.frame(summary(fit1)$summary),
            file = 'output/Reeves reckless model3 fit-summary.txt',
            sep = '\t', quote = FALSE, col.names = NA)

ggmcmc(ggs(fit1, inc_warmup = TRUE, stan_include_auxiliar = TRUE), 
       file = 'output/Reeves reckless model3 traceplot.pdf', plot = 'traceplot')
