#Recovery2: same as Recovery1 but with negative treatment effects as well
#checking parameter recovery rate by correlation
...
for(k in 1:n_sim){
  # 1. sample b1 ~ Unif(-1/14, 1/14) ; slope taking max of 1sd on day 14
  # 2. set b0 = -7*b1 ; to keep the basline mean 0      
  # 3. sample b2 ~ Unif(-0.6, 0.6); basline change
  # 4. if b2 >=0 b3 ~ Unif(0,1/14)
  #    else      b3 ~ Unif(-1/14,0); max 1 * SD change for slope                                                                                       
  # 5. sample alpha  ~ Unif(0.3, 0.8 )
  # 6. set sigma = sqrt(1-alpha^2); basline variance alpha^2 + sigma^2 = 1
  # 7. sample l ~ Unif(1,2)
  ...
  b2 = runif(1, -0.6,0.6)
  if(b2>=0){
    b3 = runif(1, 0, 1/14) #treatment effects
  }
  else{
    b3 = runif(1,-1/14,0)
  }
...
stan_model_obj <- stan_model(file = "Stan Model/Single subject semiparametric ITS with GPR simulation recovery2.stan")
...
saveRDS(results,  "output/Single subject semiparametric ITS recovery2_results.rds")
saveRDS(recovery, "output/Single subject semiparametric ITS recovery2_correlations.rds")
